...
public class CliAnnotationProcessor extends AbstractProcessor {
    ...
    private MethodSpec generateParseMethod(TypeElement typeElement,
        ...
        // 3. Generate option handlers - FIX: Check if we have options first
        if (!options.isEmpty()) {
            ...
            for (String fieldName : options.keySet()) {
                ...
                // Build condition for all option names
                ...
                // First option uses 'if', rest use 'else if'
                if (firstOption) {
                ...
                codeBuilder.add(generateOptionHandlerCode(fieldName, meta));
            ...
            if (!parameters.isEmpty()) {
                ...
                for (ParameterMetadata param : sortedParams) {
                    ...
                    codeBuilder.addStatement(FieldTypeAnalyzer.getConversionCode(fieldName, "arg", param.element.asType()));
                }
            ...
            // No options, just handle positional
    ...
    private CodeBlock generateOptionHandlerCode(String fieldName, OptionMetadata meta) {
        ...
        if (takesValue) {
            ...
            if (converterFqn != null) {
                ...
                code.addStatement("instance.$L = (($T<? extends $T>) new $L()).convert(args[idx + 1])", 
                        fieldName, 
                ...
                code.addStatement(FieldTypeAnalyzer.getConversionCode(fieldName, "args[idx + 1]", meta.element.asType()));
            }
            code.addStatement("idx++");
        } else {
            code.addStatement("instance.$L = true", fieldName);
        }
